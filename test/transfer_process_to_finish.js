...
console.log('---------------------------------------');
logger.info('Lets transfer process -', chaincode_id, chaincode_ver);
console.log('---------------------------------------');
...
fcw.enrollWithAdminCert(helper.makeEnrollmentOptionsUsingCert(), function(enrollErr, enrollResp) {
  if (enrollErr != null) {
    ...
    var processId = "test_process_001:test_linear_workflow-001";
    var nextNodeId = "test_linear_workflow-001:node-2";
    var nextOwner = "@org1.example.com";
    ...
    fcw.invoke_chaincode(enrollResp, opts, function(err, resp) {
      ...
      console.log('---------------------------------------');
      logger.info('Now we transfer process again');
      console.log('---------------------------------------');

      nextNodeId = "test_linear_workflow-001:node-3";
      nextOwner = "@org1.example.com";
      ...
      fcw.invoke_chaincode(enrollResp, opts, function(err, resp) {
        console.log('---------------------------------------');
        logger.info('transfer process again done. Errors:', (!err) ? 'nope' : err);
        console.log('---------------------------------------');
        ...
        console.log('---------------------------------------');
        logger.info('Now we transfer process to finish');
        console.log('---------------------------------------');
        ...
        fcw.invoke_chaincode(enrollResp, opts, function(err, resp) {
          console.log('---------------------------------------');
          logger.info('transfer process to finish. Errors:', (!err) ? 'nope' : err);
          console.log('---------------------------------------');
        });
