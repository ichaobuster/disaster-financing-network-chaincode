...
fcw.enrollWithAdminCert(helper.makeEnrollmentOptionsUsingCert(), function (enrollErr, enrollResp) {
  if (enrollErr != null) {
    ...
    var processInfo = {
      id: "test_process_001:test_linear_workflow-001",
    ...
    fcw.invoke_chaincode(enrollResp, opts, function (err, resp) {
      ...
      console.log('---------------------------------------');
      logger.info('Now we start another process');
      console.log('---------------------------------------');
      ...
      var processInfo = {
        id: "test_process_002:test_linear_workflow-001",
        workflowId: "test_linear_workflow-001",
        attachDocType: "project",
        attachDocId: "project-bankcomm-000002",
        createTime: new Date().toLocaleString("zh-CN")
      }
      ...
      fcw.invoke_chaincode(enrollResp, opts, function (err, resp) {
        console.log('---------------------------------------');
        logger.info('start another process done. Errors:', (!err) ? 'nope' : err);
        console.log('---------------------------------------');
      });
    });
