...
console.log('---------------------------------------');
logger.info('Lets start a process -', chaincode_id, chaincode_ver);
console.log('---------------------------------------');
...
fcw.enrollWithAdminCert(helper.makeEnrollmentOptionsUsingCert(), function (enrollErr, enrollResp) {
  if (enrollErr != null) {
    ...
    const eventHub = fcw.register_chaincode_event(enrollResp, options, (event) => {
      console.log('Successfully got a chaincode event');
      console.log('====================event====================');
      console.log(event);
      const eventObj = JSON.parse(event);
      console.log('event name: ', eventObj.eventName);
      console.log('txID: ', eventObj.txId);
      console.log('payload: ', JSON.stringify(eventObj.payload));
    });

    // 测试event

    var processInfo = {
      id: "test_process_001:test_linear_workflow-085",
      workflowId: "test_linear_workflow-001",
      attachDocType: "project",
      attachDocId: "project-bankcomm-000003",
